...
| 🗣 **语音是"会听整场会"的那种** | 不止把一句话变成文字：**听得出是谁在说**（音色自动分人）、**多台设备汇总到同一块板**、**每段都能回听**、边听边标出决定/风险/待办；识别坏了自动用浏览器自带识别兜底，并如实告诉你是哪一路。 |
...
> ![界面](docs/board.png)
>
> *静态界面图：左下是讨论面板（5 条转写 + 决定/风险/待办/依赖 四类标签 + "两个设备音色很接近"的提示），画布上是讨论中同步长出来的流程图，红色虚线是点「🎯 圈风险」标出来的风险点。*

## 语音：不是"输入框"，是一块会听会说的白板

很多白板上的"语音"= 点一下、说一句、出一个词。这里的语音分两个层面：**对着它说话**，和**让它听完整场会**。

| 能力 | 说明 |
|---|---|
| 🎤 **说完自动停** | 静音 1.3 秒自动结束并发送（不用再点一次），最长 30 秒兜底；录音转成 16k 单声道 WAV 再送去识别 |
| 🔁 **连续对话（免手动）** | 打开「连续」后，AI 朗读完会**自动重新开始听** —— 像打电话一样一来一回，不用每次点麦克风 |
| ✋ **说话即打断** | 你一开口，AI **立刻停止朗读**（不用等它念完），不会出现"它还在说、你插不上嘴" |
| 🗣 **会议级转写** | 讨论模式里持续听：静音 1.8 秒自动收段、可暂停、面板可收成小胶囊不挡画布 |
| 👥 **听得出是谁在说** | 每段算音色（基频 / 频谱质心 / 过零率）自动分人，**最多 4 人**，名字可改；**多台设备各一轨汇总到同一块板**，还会提示"这两台设备音色很接近，可能是同一个人" |
| ⏪ **每段都能回听** | 每段转写都留着**当时那段录音**：面板里点「▶ 听这段」，或在图形时间线回放时**同步播当时的声音** |
| 🏷 **边听边标记** | 转写的同时标出 🟢 决定 / 🔴 风险 / 🔵 待办 / 🟠 问题 / 🟣 依赖 |
| 🔊 **朗读默认零成本** | 用浏览器本地语音合成：不花额度、离线可用、不需要 key；想换成服务端 TTS 就在配置里填 `tts_model` |
| 🛟 **识别不绑死一家** | 服务端识别走你自己配的网关（`asr_model`）；**额度用尽 / 连不上 / 没配**时自动改用浏览器自带识别顶上，并且界面上写清这句是"浏览器识别" —— **不偷偷降级** |
| 🔒 **需要 HTTPS** | 浏览器只在安全上下文里给麦克风；页面检测到 HTTP 时会提示，并给一键切到 `https://…:9443` |

> 上面那张演示 GIF 是**无声**的：真实使用时，讨论面板里那些逐条出现的转写，就是对着麦克风说话产生的。
> 音频去哪了：服务端识别会把录音发给你配置的那家语音网关；走浏览器兜底时由浏览器厂商在线识别。详见 `SECURITY.md`。
...
| 语音 | 说完自动停、**连续对话免手动**、**说话即打断**；**听得出是谁在说**、多台设备汇总到一块板、每段都能回听（详见上面「语音」一节）；服务端识别不可用时自动用浏览器识别兜底并如实标注 |
