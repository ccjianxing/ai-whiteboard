...
- 单文件前端：`board.html`（原生 JS，无框架、无构建）
- 零依赖后端：`server_v2.py`（**纯 Python 标准库**，不用 pip 装任何东西）
- 给 agent 的接口：`mcp_server.py`（标准 MCP，DSH / Claude Code / Cursor 都能接）
- 运行环境：**Python 3.8+**（Ubuntu 20.04 自带的就是 3.8，直接能跑）、任意现代浏览器

---


![AI 白板](docs/board.png)

*画板实景：模板画的流程图；右上「绿底 A 角标」表示这个元素是你的 agent 画的；
面板顶部显示「我的助手 已接入」——此时你在这里说话就是跟它对话。*
...
> 📖 想让 agent 自动入驻看下一节；部署与鉴权看「部署到服务器」；常见问题在文末「安全提醒」。
...
举个例：agent 拿到 `http://<服务器>:<端口>/join.md`，按里面三步就能开始工作：
...
| 多画板 | 画板管理与切换、跨板引用、对比、演示模式 |
| 画板管理 | 画板栏标签**右键**就有：重命名 / 复制这个画板 / 删除 / 前移 / 后移 / 移到最前 / 移到最后 / 从这页演示 / 导出这页 PNG；**删除可撤销**（顶部提示条 12 秒内点一下就回来）；`Alt+1..9` 快速切画板 |
| 工具栏分组 | 左栏按类型分成 **6 组**（基础 / 线条 / 图形 / 流程 / 结构 / 其他），**每组可折叠**（状态记住）；流程与结构符号原来收在弹出菜单里，现在展开就能看到 |
| 元素面板 | 顶栏「▤ 元素」：当前画板上的元素**按类型分组**（数量多的排前面），**每组可折叠**（状态会记住），点条目即选中并定位过去，点数量选中整组 |
...
| 语音 | 麦克风 → 服务端识别（**需要 HTTPS**：浏览器只在安全上下文里给麦克风）；**服务端识别不可用时（比如网关额度用尽）自动用浏览器自带识别顶上，并在界面上写清这句是"浏览器识别"** |
| 传文件给 AI | 对话里点 📎 或把文件**直接拖进输入框**：AI/agent 先读你的方案（markdown/txt/json/csv…），再照着画；支持按行分段读长方案，收 40 万字 |
...
不同的板互不可见：agent 归属、对话、工具调用都按板隔离；`?board=` 为空按 `default` 板算（**不做通配**，
否则一台旧页面就能把别人板上的工具调用抢走 —— 这个洞实测抓到过）。
...
| `board_chat_read(limit)` | 读整段对话（用户说的 + 其它 agent 说的） |
| `board_chat_say(text)` | 对白板说话，用户直接看到 |
...
避免"实现了却忘了告诉 AI"这类漂移（`工具同步自检.py` 会强制两者一致）。
...
这个仓库只有跑起来必需的东西，一共 8 个文件：

```
board.html          单文件前端（含全部 CSS/JS，无框架、无构建）
server_v2.py        后端（纯 Python 标准库；同步、账号、对话、待办、附件、MCP 桥、内置 AI 调用）
...
> 本项目 1.0.4 起改成静态**白名单**（只发页面 `/board.html`、`/` 与 `docs/board.png`），并且不再生成目录清单。
...
- **界面**：顶栏 **🌐 EN**（在「更多 ▾」右边）一键切换，选择会记住。词表 **843 条**，覆盖
  按钮、**全部下拉菜单**（文件/模板/整理/代码/画板/导出/更多，含模板名与开关状态）、面板标题、
  下拉选项、悬停提示、同步条 / agent 条 / 输入框提示等动态文案。
  菜单是点开时才生成的，所以 popMenu 生成后立刻翻一次 —— 这点漏过一回。
- **仍是中文的部分**：少数长帮助文本（悬停才看到的整段说明）、罕见 toast 与边界错误、服务端返回的错误信息。
  语言表的键就是"中文原文"，**没翻到的保持中文，不会变空白**。
- **默认中文，不自动跟随浏览器语言**：默认值必须确定 —— 否则截图基线与自动化会随环境漂移。
  要英文，点一下顶栏的 EN。
...
- **静态文件只发白名单**（页面 + 截图），其余一律 404 —— 这是刻意设计，改这段代码前请先读上面的「自己验证」。
